...
it('calculates price and discount', async () => {
  ...
  */
  ...
  // semaphor to prevent downstream changes
  const semaphorAtom = atom(() => ({ value: false }))

  // reacts to changes to unitPrice
  const unitPriceAtom = withAtomEffect(atom(100), (get, set) => {
    const unitPrice = get(unitPriceAtom)
    const price = get.peek(priceAtom) // don't subscribe to priceAtom
    const discount = get.peek(discountAtom) // don't subscribe to discountAtom
    const nextPrice = unitPrice * (1 - discount / 100)
    set(priceAtom, nextPrice)
    const nextDiscount = ((unitPrice - price) / unitPrice) * 100
    set(discountAtom, nextDiscount) // recalculate price if discount is 20 or 80
  })
  ...
  const discountAtom = withAtomEffect(atom(0), (get, set) => {
    ...
    const unitPrice = get.peek(unitPriceAtom) // don't subscribe to unitPriceAtom

...
const hasInitialValue = <T extends Atom<AnyValue>>(
  atom: T
): atom is T & (T extends Atom<infer Value> ? { init: Value } : never) => 'init' in atom
...
const returnAtomValue = <Value>(atomState: AtomState<Value>): Value => {
  ...
  if (process.env?.MODE !== 'production' && !('v' in atomState)) {
...
const addDependency = <Value>(
  ...
  if (process.env?.MODE !== 'production' && a === atom) {
...
const buildStore = (...storeArgs: StoreArgs): Store => {
  const [getAtomState, setAtomState, atomRead, atomWrite, atomOnInit, atomOnMount] = storeArgs
  const ensureAtomState = <Value>(atom: Atom<Value>) => {
    if (process.env?.MODE !== 'production' && !atom) {
  ...
  const readAtomState = <Value>(atom: Atom<Value>): AtomState<Value> => {
    ...
    const options = {
      ...
      get setSelf() {
        if (process.env?.MODE !== 'production' && !isActuallyWritableAtom(atom)) {
          console.warn('setSelf function cannot be used with read-only atom')
        }
        if (!setSelf && isActuallyWritableAtom(atom)) {
          setSelf = (...args) => {
            if (process.env?.MODE !== 'production' && isSync) {
  ...
  const recomputeInvalidatedAtoms = () => {
    ...
    const topSortedReversed: [atom: AnyAtom, atomState: AtomState, epochNumber: EpochNumber][] = []
  ...
  const mountAtom = <Value>(atom: Atom<Value>, atomState: AtomState<Value>): Mounted => {
    if (!atomState.m) {
  ...
  const unstable_derive: Store['unstable_derive'] = (fn) => buildStore(...fn(...storeArgs))
